...
{
	ASIHTTPRequest *request = [[self alloc] initWithURL:newURL];
...
#if TARGET_OS_IPHONE
			NSDictionary *proxySettings = (__bridge NSDictionary *)((__bridge CFTypeRef)((__bridge NSDictionary *)CFNetworkCopySystemProxySettings()));
#else
			NSDictionary *proxySettings = NSMakeCollectable([(NSDictionary *)SCDynamicStoreCopyProxies(NULL) autorelease]);
#endif
			
			proxies = (__bridge NSArray *)((__bridge CFTypeRef)((__bridge NSArray *)CFNetworkCopyProxiesForURL((__bridge CFURLRef)[self url], (__bridge CFDictionaryRef)proxySettings)));
...
//		[oldStream release];
	...
	if (![self mainRequest]) {
		if ([self shouldResetUploadProgress]) {
            ...
            id uploadProgressDelegate_t = uploadProgressDelegate;
			[ASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate_t withProgress:0 ofTotal:1];
		}
		if ([self shouldResetDownloadProgress] && ![self partialDownloadSize]) {
            id downloadProgressDelegate_t = downloadProgressDelegate;
			[ASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate_t withProgress:0 ofTotal:1];
		}
	}	
...
- (void)checkRequestStatus
{
	...
	if ([self readStream]) {
		...
		if ([self postLength]) {
			...
			[self setTotalBytesSent:[((NSNumber *)CFBridgingRelease(CFReadStreamCopyProperty((CFReadStreamRef)[self readStream], kCFStreamPropertyHTTPRequestBytesWrittenCount))) unsignedLongLongValue]];
...
- (ASIHTTPRequest *)HEADRequest
{
	...
	[headRequest setClientCertificates:[clientCertificates copy]];
...
- (void)updateDownloadProgress
{
    ...
    id <ASIHTTPRequestDelegate, ASIProgressDelegate> queue_t = queue;
    id downloadProgressDelegate_t = downloadProgressDelegate;
	[ASIHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&queue_t withObject:self amount:&value];
	[ASIHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&downloadProgressDelegate_t withObject:self amount:&value];
	[ASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate_t withProgress:[self totalBytesRead]+[self partialDownloadSize] ofTotal:[self contentLength]+[self partialDownloadSize]];
...
- (void)updateUploadProgress
{
	...
	[ASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue_t withObject:self amount:&value];
	[ASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate_t withObject:self amount:&value];
	[ASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate_t withProgress:[self totalBytesSent]-[self uploadBufferSize] ofTotal:[self postLength]-[self uploadBufferSize]];
}
...
- (void)incrementDownloadSizeBy:(long long)length
{
    id queue_t = queue;
    id downloadProgressDelegate_t = downloadProgressDelegate;
	[ASIHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&queue_t withObject:self amount:&length];
	[ASIHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&downloadProgressDelegate_t withObject:self amount:&length];
}
...
- (void)incrementUploadSizeBy:(long long)length
{
    id queue_t = queue;
    id uploadProgressDelegate_t = uploadProgressDelegate;
	[ASIHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&queue_t withObject:self amount:&length];
	[ASIHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&uploadProgressDelegate_t withObject:self amount:&length];
}
...
-(void)removeUploadProgressSoFar
{
	...
	[ASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue_t withObject:self amount:&progressToRemove];
	[ASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate_t withObject:self amount:&progressToRemove];
	[ASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate_t withProgress:0 ofTotal:[self postLength]];
}
...
- (void)callSelectorOnMainThread:(SEL *)selector forDelegate:(id *)target
{
	...
	[invocation setArgument:(void*)&self atIndex:4];
...
- (void)requestReceivedResponseHeaders
{
    ...
    id delegate_t = delegate;
    id queue_t = queue;
	[self callSelectorOnMainThread:&didReceiveResponseHeadersSelector forDelegate:&delegate_t];
	
	// Let the queue know we have started
	[self callSelectorOnMainThread:&queueRequestReceivedResponseHeadersSelector forDelegate:&queue_t];
}
...
- (void)requestStarted
{
    ...
    id queue_t = queue;
	// Let the delegate know we have started
	[self callSelectorOnMainThread:&didStartSelector forDelegate:&delegate_t];
	
	// Let the queue know we have started
	[self callSelectorOnMainThread:&queueRequestStartedSelector forDelegate:&queue_t];
}
...
- (void)requestFinished
{
	...
	[self callSelectorOnMainThread:&didFinishSelector forDelegate:&delegate_t];
	
	// Let the queue know we are done
	[self callSelectorOnMainThread:&queueRequestFinishedSelector forDelegate:&queue_t];
}
...
- (void)reportFailure
{
	...
	[self callSelectorOnMainThread:&didFailSelector forDelegate:&delegate_t];
	
	// Let the queue know something went wrong
	[self callSelectorOnMainThread:&queueRequestFailedSelector forDelegate:&queue_t];
}
...
- (void)readResponseHeaders
{
	...
	[self setResponseHeaders:(__bridge NSDictionary *)headerFields];

	CFRelease(headerFields);
	
	[self setResponseStatusCode:(int)CFHTTPMessageGetResponseStatusCode(message)];
	[self setResponseStatusMessage:(__bridge NSString *)CFHTTPMessageCopyResponseStatusLine(message)];
...
{
	NSMutableDictionary *newCredentials = [[NSMutableDictionary alloc] init];
...
- (void)attemptToApplyProxyCredentialsAndResume
{
	...
	if (!proxyAuthentication) {
		...
		[self setProxyAuthenticationScheme:(__bridge NSString *)CFHTTPAuthenticationCopyMethod(proxyAuthentication)];
	}
	...
	if (!CFHTTPAuthenticationRequiresAccountDomain(proxyAuthentication)) {
		[self setProxyAuthenticationRealm:(__bridge NSString *)CFHTTPAuthenticationCopyRealm(proxyAuthentication)];
	}
...
- (void)attemptToApplyCredentialsAndResume
{
	...
	if (!requestAuthentication) {
		...
		CFRelease(responseHeader);
		[self setAuthenticationScheme:(__bridge NSString *)CFHTTPAuthenticationCopyMethod(requestAuthentication)];
	}
	...
	if (!CFHTTPAuthenticationRequiresAccountDomain(requestAuthentication)) {
		[self setAuthenticationRealm:(__bridge NSString *)CFHTTPAuthenticationCopyRealm(requestAuthentication)];
	}
	...
	// If zero bytes were read, wait for the EOF to come.
		...
		// Are we downloading to a file?
			...
			if (![self fileDownloadOutputStream]) {
				...
				[self setFileDownloadOutputStream:[[NSOutputStream alloc] initToFileAtPath:[self temporaryFileDownloadPath] append:append]];
...
{
	NSError *underlyingError = (NSError *)CFBridgingRelease(CFReadStreamCopyError((CFReadStreamRef)[self readStream]));
...
- (id)copyWithZone:(NSZone *)zone
{
	...
	[newRequest setRequestHeaders:[[self requestHeaders] mutableCopyWithZone:zone]];
	[newRequest setRequestCookies:[[self requestCookies] mutableCopyWithZone:zone]];
	...
	[newRequest setUserInfo:[[self userInfo] copyWithZone:zone]];
	...
	[newRequest setClientCertificates:[clientCertificates copy]];
...
{
	NSURLProtectionSpace *protectionSpace = [[NSURLProtectionSpace alloc] initWithHost:host port:port protocol:protocol realm:realm authenticationMethod:NSURLAuthenticationMethodDefault];
...
{
	NSURLProtectionSpace *protectionSpace = [[NSURLProtectionSpace alloc] initWithProxyHost:host port:port type:NSURLProtectionSpaceHTTPProxy realm:realm authenticationMethod:NSURLAuthenticationMethodDefault];
	[[NSURLCredentialStorage sharedCredentialStorage] setDefaultCredential:credentials forProtectionSpace:protectionSpace];
}
...
{
	if (!sessionCookies) {
		[ASIHTTPRequest setSessionCookies:[[NSMutableArray alloc] init]];
	}
...
+ (NSArray *)proxiesForURL:(NSURL *)theURL fromPAC:(NSURL *)pacScriptURL
{
	...
	CFRelease(CFNetworkCopyProxiesForURL((__bridge CFURLRef)theURL, NULL));
	...
	NSArray *proxies = (__bridge NSArray *)CFNetworkCopyProxiesForAutoConfigurationScript((__bridge CFStringRef)script,(__bridge CFURLRef)theURL, &err2);
	if (err2) {
		return nil;
	}
	return proxies;
...
+ (NSString *)mimeTypeForFileAtPath:(NSString *)path
{
    ...
    return (__bridge NSString *)((__bridge CFTypeRef)((__bridge NSString *)MIMEType));
}
