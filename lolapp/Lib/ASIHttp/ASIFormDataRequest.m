...
{
	NSString *newString = (__bridge NSString *)((__bridge CFTypeRef)((NSString *)CFBridgingRelease(CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, (CFStringRef)string, NULL, CFSTR(":/?#[]@!$ &'()*+,;=\"<>%{}|\\^~`"), CFStringConvertNSStringEncodingToEncoding([self stringEncoding])))));
	if (newString) {
		return newString;
	}
	return @"";
